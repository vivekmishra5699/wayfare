...
class TilesetExecutorPreprocessor {
  ...
  Future<Tileset> preprocess(TileIdentity identity, Tileset tileset,
    ...
    // Patched (open_maps): the executor is shared by every layer on the map,
    // so the key must include the theme — otherwise two layers with
    // different themes (e.g. geometry-only and symbols-only) over the same
    // tile are deduplicated together and one gets the other's result.
    final deduplicationKey =
        'preprocess: $identity theme=${_preprocessor.theme.id} clip=$clip zoom=$zoom sources=${tileset.tiles.keys.toList().sorted().join(',')}';
