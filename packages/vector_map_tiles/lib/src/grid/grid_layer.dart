...
class _VectorTileCompositeLayerState extends State<VectorTileCompositeLayer>
  ...
  void initState() {
    super.initState();
    // Patched (open_maps): the state mixes in WidgetsBindingObserver for
    // didHaveMemoryPressure but was never registered, so the caches never
    // heard about memory pressure.
  ...
  Widget build(BuildContext context) {
    ...
    if (options.layerMode == VectorTileLayerMode.raster) {
      ...
      final tileLayer = TileLayer(
          ...
          // Patched (open_maps): a slightly longer cross-fade reads as
          // continuity rather than popping when tiles arrive mid-gesture.
          tileDisplay:
              const TileDisplay.fadeIn(duration: Duration(milliseconds: 220)),
          tileProvider: tileProvider);
      if (backgroundTheme == null) {
        return tileLayer;
      }
      // Patched (open_maps): land/water from whatever coarser tile is
      // already cached, painted under the bitmaps so a zoom never shows a
      // blank page while tiles render (the vector mode has always had this).
      return Stack(children: [
        MobileLayerTransformer(child: _backgroundLayer(backgroundTheme)),
        tileLayer,
      ]);
    }
    final layers = <Widget>[];
    if (backgroundTheme != null) {
      layers.add(_backgroundLayer(backgroundTheme));
    }
