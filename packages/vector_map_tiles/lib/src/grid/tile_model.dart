...
class _VectorTileModelLoader {
  ...
  void startLoading() async {
    ...
    if (!model.disposed) {
      try {
        ...
        // Patched (open_maps): if the vector request is cancelled while the
        // raster tiles are still being awaited below, its error has no
        // listener yet and is reported as an unhandled exception. Attach a
        // no-op handler; the real `await loading` still sees the error.
        loading.ignore();
  ...
  TileRequest _newRequest() {
    ...
    return TileRequest(
        ...
        // Patched (open_maps): `model.theme` is the geometry-only theme; a
        // symbols-only layer would otherwise request no sources at all.
        tileSources: {
          ...model.theme.tileSources,
          ...?model.symbolTheme?.tileSources,
        },
        zoom: zoom.zoom,
