...
class VectorTileLayer extends StatelessWidget {
  ...
  /// Patched (open_maps): pixels rendered per logical tile pixel in raster
  /// [layerMode]. Match the device pixel ratio for crisp bitmaps; 2.0 was
  /// the previous fixed value.
  final double rasterScale;

  /// Patched (open_maps): byte budget for rendered tile bitmaps kept in
  /// memory in raster [layerMode], so zoom levels that were recently on
  /// screen come back without being rendered again. Bitmaps that are also
  /// on screen share their pixels with the map and cost nothing extra here,
  /// so size this to the level on screen plus the neighbouring levels a
  /// zoom would need: a few screenfuls of RGBA device pixels.
